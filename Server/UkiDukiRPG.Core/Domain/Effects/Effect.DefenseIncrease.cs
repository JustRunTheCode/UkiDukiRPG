...
) : BuffEffect(nameof(DefenseIncreaseEffect), StatusEffectType.DefenseIncrease, duration)
// @formatter:on
{
    ...
    public override void Apply(Combatant caster, Combatant target, IBattleEngine battle)
    {
        ...
        m_Amount = (int)float.Round((m_BaseIncrease + target.DefenseLevel * m_IncreaseFactor) * casterModifier * targetModifier);
    ...
    public override void Clear(Combatant combatant)
    {
        ...
        combatant.DescendAttribute(AttributeType.Defense, m_Amount);
    }
}
